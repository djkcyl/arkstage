...
- **D3 网络策略 = 全局「允许联网」开关 + 缺失穿透回退**：
  - 允许联网时：`prts-cdn://` 对缺失媒体资源直接从 prts 拉取并缓存到本地（cache-through），照顾 PC / 常驻无限网用户，无需预缓存也能即看即存。
  - 不允许联网时：命中本地即离线播放；若缺失，则不发出站请求，并提示用户「开启联网」或「先用预缓存功能获取资源」。
...
- **`prts-cdn://` 协议改造为离线优先**：解析出 `{host}/{path}` 后，先查 `media/{host}/{path}`，命中即从磁盘返回（离线）；未命中且**允许联网**时，拉取并落盘后返回（穿透回退）；未命中且**不允许联网**时返回可识别的失败响应（如 503 + 自定义头），前端据此提示用户开启联网或先预缓存。
- **「允许联网」开关**：全局设置，由 Rust 侧共享状态（如 `AtomicBool`，启动时从持久化设置读入）持有，因为协议 handler 在 Rust 中运行、无 JS 上下文；前端经命令 `set_allow_online` 切换。
...
- `media` 存储模块（新增）
  ...
  - 复用 `lib.rs` 的 `prts-cdn://` handler：改为先查 `media/{host}/{path}`，命中读盘返回；未命中且允许联网走现有 reqwest 拉取、**成功后写入** `media/{host}/{path}` 再返回；未命中且不允许联网返回可识别失败响应。
- 「允许联网」共享状态（新增）：`AtomicBool`（Tauri `State` / `OnceLock`），启动从持久化设置读入；命令 `set_allow_online(bool)` / `get_allow_online()`；handler 读取它决定是否穿透。
...
- **Phase 2 — 媒体仓库 + 离线优先协议**：内容寻址存储；「允许联网」共享状态与 `set_allow_online` 命令；`prts-cdn://` 先本地、允许时穿透落盘、不允许时返回可识别失败 + 前端提示；改造 `batch_download_assets`。
...
- 不允许联网且资源缺失时，前端如何稳定捕获失败并提示（监听媒体 error 事件 / 协议返回的自定义头 / 统一拦截）。
- 进度回报用 Tauri 事件流还是轮询。
